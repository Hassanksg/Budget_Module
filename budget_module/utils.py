...
from flask import session, has_request_context, current_app, request, url_for, redirect, flash
...
def get_explore_features():
    """Return explore features for unauthenticated users on the landing page with resolved URLs."""
    try:
...
def log_tool_usage(action, tool_name=None, details=None, user_id=None, db=None, session_id=None):
    ...
    Args:
        action (str): The action performed (e.g., 'create_budget').
...
def clean_currency(value, max_value=10000000000):
    """
    Clean currency input by removing non-numeric characters, handling various currency formats.
...
__all__ = [
    ...
    'get_mongo_db', 'close_mongo_db', 'requires_role',
